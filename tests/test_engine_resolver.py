...
def test_resolve_from_inventory():
    engine = make_engine()
    gun = Item(
        id="gun", name="pistola", base_description=".", synonyms=["pistola", "arma"]
    )
...
def test_resolve_prefers_inventory_over_room():
    engine = make_engine()
    inv_gun = Item(
        id="gun_inv", name="pistola", base_description=".", synonyms=["pistola"]
    )
    room_gun = Item(
        id="gun_room", name="pistola", base_description=".", synonyms=["pistola"]
    )
    engine.current_player = MagicMock()
    engine.current_player.inventory = [inv_gun]
    engine.current_player.current_location = "jazz_club"
    loc = Location(
        id="jazz_club", name="Club", base_description=".", children=["gun_room"]
    )
    engine.locations = {"jazz_club": loc}
...
def test_resolve_from_open_container():
    ...
    contract = Item(
        id="contract", name="contrato", base_description=".", synonyms=["contrato"]
    )
    desk = Item(
        id="desk",
        name="escritorio",
...
def test_resolve_door_via_local_globals():
    """Bug #5 regression: doors listed in location's local_globals must be resolvable."""
    engine = make_engine()
    main_door = Door(
        id="main_door",
        name="puerta principal",
        base_description="Una puerta pesada.",
        synonyms=["puerta", "entrada", "salida"],
        connects=("jazz_street", "jazz_club"),
        flags={GameFlag.DOOR, GameFlag.OPEN},
    )
    engine.current_player = MagicMock()
    engine.current_player.inventory = []
    engine.current_player.current_location = "jazz_club"
    loc = Location(
        ...
        base_description=".",
        children=[],
        local_globals=["main_door"],
    )
    engine.locations = {"jazz_club": loc}
    engine.items = {}
    engine.npcs = {}
    engine.door_registry.get.side_effect = lambda door_id: (
        main_door if door_id == "main_door" else None
    )

    result = engine._resolve_object("puerta")
    assert result is not None
    assert result.id == "main_door"
...
def test_matches_object_by_synonym():
    ...
    engine.door_registry = MagicMock()
    item = Item(
        id="gun", name="pistola", base_description=".", synonyms=["arma", "revólver"]
    )

...
class AIEnhancer(ABC):
    ...
    def enhance_description(
        self, base_description: str, context: Dict[str, Any]
    ) -> str:
    ...
    @abstractmethod
    def enhance_usage(
        self, object: str, action: str, target: str, result: Dict[str, Any]
    ) -> str:
...
class ClaudeEnhancer(AIEnhancer):
    ...
    def enhance_examine(self, target, context: Dict[str, Any]) -> str:
        ...
        messages = [
            SystemMessage(
                content="""You are enhancing descriptions for a GUMSHOE mystery game. 
    ...
    def enhance_usage(
        self, object_name: str, action: str, target: str, result: Dict[str, Any]
    ) -> str:
        """Enhance the description of action results"""
        messages = [
            SystemMessage(
                content="""You are describing the results of player actions in a GUMSHOE mystery game.
    ...
    def summarize_conversation(
        ...
        messages = [
            SystemMessage(
                content="""Summarize this conversation between a player and NPC in a mystery game.
...
class OpenAISimpleAIEnhancer(AIEnhancer):
    ...
    def interpret_command(
        ...
        return {
            "action": "examine",
            "target": command.split()[-1] if command.split() else "room",
            "confidence": 0.8,
        }
    ...
    def summarize_conversation(
        ...
        conversations_text = "\n".join(
            [f"{entry.speaker}: {entry.text}" for entry in conversation_history]
        )
